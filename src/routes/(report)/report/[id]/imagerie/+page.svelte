					...
					<tr class="p-0 m-0">
						...
						<td class="p-0 m-0">
							អាយុ {dobToAge({
								...
								m: new Date(get_imagerie_request?.visit?.patient?.dob ?? '').getMonth() + 1,
								y: new Date(get_imagerie_request?.visit?.patient?.dob ?? '').getFullYear(),
					...
					<tr class="p-0 m-0">
						...
						<td class="p-0 m-0"
							>{new Intl.DateTimeFormat('en-GB', {
								...
								hour12: true,
								timeStyle: 'short'
							}).format(new Date(get_imagerie_request?.finish_datetime || new Date()))}</td
						>
					</tr>
					<tr class="p-0 m-0">
						<td style="font-size: 110%;" class="text-bold en_font_times_new_roman p-0 m-0">Phone</td
						>
						<td class="p-0 m-0"> : </td>
...
<main class="content">
	<u>
		<h1 style="font-size: 130%;" class="text-center kh_font_muol">លទ្ធផលរូបភាពវេជ្ជសាស្ត្រ</h1>
		<h1 style="font-size: 100%;" class="text-center en_font_times_new_roman">Imagerie Result</h1>
	</u>
	<div>
	...
	<div class="row">
		{#each get_imagerie_request?.fileOrPicture || [] as item}
			<div class="col-4 p-2">
				<img style="width: 100%;height: 100%;" src="/files/{item.filename}" alt="some alt text" />
			</div>
		{/each}
	</div>

	<!-- Forces a new page when printing -->
</main>
